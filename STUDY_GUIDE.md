...
- **Types of Agents:**
  - **Simple Reflex:** Action based on immediate perception (e.g., thermostat).
  - **Goal-Based:** Planning to achieve a specific goal (e.g., GPS navigation).
  - **Utility-Based:** Maximizing a utility function (trade-offs like speed vs. cost).
  - **Learning Agents:** Improving performance over time through feedback.
  - **Multi-Agent Systems:** Collaboration between multiple agents to solve complex problems.

## 2. Agentic Frameworks
**Goal:** Explore the tools available for building agents.

**Key Frameworks:**
- **Semantic Kernel:** Production-ready SDK (C#/Python) for integrating LLMs with code. Good for enterprise applications, offering plugins, planners, and memory connectors.
- **AutoGen:** Framework for multi-agent conversation and orchestration. Excellent for research and complex, distributed agent patterns where agents converse to solve tasks.
- **Azure AI Agent Service:** Managed cloud service for deploying secure, scalable agents. Simplifies hosting and management.
...
- **Time:** Agents should reflect on the **Past** (memory), nudge in the **Now** (proactive assistance), and adapt for the **Future** (planning).
...
- **Consistency:** Predictable behavior and interfaces to build user confidence.
...
- **Use Cases:** Retrieving real-time data (weather, stock prices), performing calculations, executing code, interacting with APIs (sending emails, querying databases).
...
- **Threat Modeling:**
  - **Input Manipulation:** Prompt injection (trying to override instructions).
  - **Resource Overloading:** Denial of Service.
  - **Knowledge Poisoning:** Corrupted data sources.
- **Mitigation:** Input validation, access controls, human-in-the-loop (approval steps) for high-stakes actions.
...
- **Implementation:** Prompting the agent to "reflect" on its last decision before finalizing it (e.g., "Critique your last answer").
...
**Evaluation:**
- **Offline:** Testing against curated datasets (Golden Datasets) to ensure baseline performance.
- **Online:** Monitoring real user interactions and feedback to detect drift or issues.
...
**Types:**
- **Working Memory:** Scratchpad for immediate tasks.
- **Short-Term:** Context of the current conversation.
- **Long-Term:** Persistent facts and preferences across sessions.
- **Episodic:** Remembering sequences of actions/events.
- **Persona:** Consistent personality traits.
- **Entity Memory:** Structured facts about people/places.

**Implementations:**
- **Mem0:** Specialized memory layer for agents.
- **Cognee:** Knowledge graph + Vector store for semantic and relational recall.
- **Structured RAG:** Extracting entities into databases for precise retrieval.
...
**Key Components:**
- **Executors:** Perform tasks (Agent or Code).
- **Edges:** Define flow (Direct, Conditional, Switch-case, Fan-out, Fan-in).
- **Events:** For tracking execution (Start, Complete, Error).
